...
const Terminal = () => {
  ...
  const executeCommand = (command: string) => {
    ...
    switch (mainCommand) {
      case "cd":
        if (argument === ".." && newPath.length > 1) {
          ...
          if (currentDir[folder]) {
            ...
            newOutput.push(
              `>> The system cannot find the path specified: ${folder}`
            );
          }
        }
        break;
      ...
      case "dir": {
        ...
        for (const part of newPath.slice(1)) {
          if (currentDir[part]) {
            ...
            newOutput.push(
              `>> The system cannot find the path specified: ${newPath.join(
                "\\"
              )}`
            );
      ...
      case "riddle":
        ...
        setTimeout(
          () => newOutput.push(`Answer: ${randomRiddle.answer}`),
          2000
        );
        break;
  ...
  const closeTab = (id: number) => {
    ...
    if (activeTab === id) {
      ...
      const nextActive =
        remainingIds.find((tabId) => tabId > id) ||
  ...
  return (
    <div className="w-full h-full flex flex-col text-lg bg-black text-white font-mono overflow-hidden rounded shadow-lg border border-gray-700">
      ...
      <div className="flex bg-gray-800 text-gray-300 p-2 space-x-2">
        {tabs.map((tab) => (
          <div
            ...
            className={`px-4 py-1 cursor-pointer ${
              activeTab === tab.id ? "bg-gray-700 text-white" : "bg-gray-900"
            }`}
        ...
        <button className="px-4 py-1 bg-gray-700" onClick={addNewTab}>
          ➕
        </button>
      </div>
